...
function buildDisplayPaths(hosts) {
    for (var hostName in hosts) {
        ...
        if (host.paths.length > 1) {
            ...
            var matched;
            while (ptr > -1) {
                matchString = pathname.substr(0, ptr);

                matched = host.paths.every(function(path) {
            ...
            while (ptr > -1) {
                matchString = pathname.substr(ptr);
...
function buildHostMap(hostPathPairs) {
    ...
    hostPathPairs.forEach(function(pair) {
        ...
        if (hostAndPair.length !== 2) {
            console.log('Failed to parse host ' + pair);
            console.log('Expected a host/path pair like this:');
            console.log('trillworks.com:/var/log/logfile.log');
            process.exit(1);
        }
